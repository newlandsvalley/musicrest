...
class AbcSpec extends Specification {
  ...
  "The ABC parser" should {    
    "parse well formed ABC" in {  
        ...
        v.fold(e => 
           validationFailure("abc parse failed")
         , s => {          
    ...
    "Add a further title" in {      
        ...
        v.fold(e => 
            ...
            , oldAbc => {
                ...
                newAbc.fold (e =>   
                    validationFailure("couldn't add new title")   
                , s => {
   ...
   "Disallow adding a non-Unicode title" in {      
        ...
        v.fold(e => 
             ...
             , oldAbc => {
                ...
                newAbc.fold (
                 ...
                 , s => {     
                     validationFailure("allowed a non-unicode title")   
                })
        })      
    }        
    ...
    "Reject a tune with no title" in {      
        ...
        v.fold(e => 
           e must_== ("No title (T header) present in abc")
         , s => validationFailure("accepted title-less tune")  
         )
    }

    "Reject a tune with a title not in Unicoode" in {      
        val v = getValidTune(badData.notUnicode)
        v.fold(e => e must_== ("Please use Unicode for all titles and don't use backslashes - you have submitted Polska fran H\\'alleforsn\\'as")
             , s => validationFailure("accepted tune with title not in Unicode" ))
    }
    "Reject a tune with no key signature" in {      
        val v = getValidTune(badData.keyless)
        v.fold(e => e must_== ("No key Signature present in abc"), s => validationFailure("accepted key-less tune" ))
    }
    "Reject a tune with a bad key signature" in {      
        val v = getValidTune(badData.badkey)
        v.fold(e => e must_== ("Unrecognized key signature: Baol"), s => validationFailure("accepted tune with unknown key signature" ))
    }
    "Reject a tune with a bad rhythm" in {      
        val v = getValidTune(badData.badrhythm)
        v.fold(e => e must_== ("foo is not a recognized rhythm for the irish genre"), s => validationFailure("accepted tune with unknown rhythm" ))
    }
    "Suppress self references in tune sources" in {      
        ...
        v.fold(e => validationFailure("parse should succeed but failed: " + e), s => {
           s.source must_== (None)
        })        
    }
  }
  
  def getValidTune(content: String): Validation[String, Abc] = {
